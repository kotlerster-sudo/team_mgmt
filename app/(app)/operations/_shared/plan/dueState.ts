 ...
 * "Today" worklist (overdue + today only, critical-path first) that is the setup-centre default.
 * Lives client-side on purpose: the plan loader runs on the server (UTC on Vercel) so any "is
 * today" comparison there would drift from the RP's local day; local Y/M/D comparison here
 * matches the home module's isToday() idiom (never iso.slice(0,10)).
 */
...
export type TodayNode = {
  ...
  waitingOnWbs: string | null; // set when this node can't be acted on yet (a predecessor is open)
  itemsDone: number;
  itemsTotal: number;
};

export type TodayViewData = {
  overdue: TodayNode[]; // each ranked: critical-path first, then earliest targetDate, then wbs
  today: TodayNode[];
...
export function buildTodayView(plan: CentrePlan): TodayViewData {
  ...
  const wbsOfFirstOpenPred = (n: PlanNode): string | null => {
    const openPred = n.blockedBy.find((id) => !doneIds.has(id));
    return openPred ? byId.get(openPred)?.wbs ?? "?" : null;
  };
  ...
  const toTodayNode = (n: PlanNode, due: NonNullable<DueState>): TodayNode => ({
    pitstopId: n.pitstopId, wbs: n.wbs, title: n.title, status: n.status, ownerName: n.ownerName,
    due, onCriticalPath: n.onCriticalPath, waitingOnWbs: wbsOfFirstOpenPred(n),
    itemsDone: n.subItems.filter((s) => s.doneCount >= s.totalCount).length,
    itemsTotal: n.subItems.length,
  });

  const rank = (a: TodayNode, b: TodayNode) => {
  ...
  const overdue: TodayNode[] = [];
  const today: TodayNode[] = [];
  for (const n of allNodes) {
    const due = getDueState(n.targetDate, n.status);
    if (!due) continue;
    (due.kind === "overdue" ? overdue : today).push(toTodayNode(n, due));
  }
  overdue.sort(rank);
  today.sort(rank);
  ...
  let waitingOn: TodayViewData["waitingOn"] = null;
  if (nearest) {
    if (actionable(nearest)) nextUpId = nearest.pitstopId;
    else waitingOn = { nodeId: nearest.pitstopId, onWbs: wbsOfFirstOpenPred(nearest) ?? "?" };
  }

  return { overdue, today, nextUpId, waitingOn };
}
