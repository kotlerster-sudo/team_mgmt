...
export default function BudgetEditor({ budget }: { budget: Budget }) {
  ...
  const handleDelete = (lineId: string) => {
    // Deleting a line cascades to any filed report lines, so the server refuses
    // past draft. Block here too — the row removal below is optimistic and would
    // otherwise hide a line the server kept.
                    ...
                    <Fragment key={l.id}>
                      {editing === l.id
                        ...
                        : <ViewRow line={l} i={i + 1} bands={bands} showAlloc={showAlloc}
                            onEdit={() => startEdit(l)} onDelete={() => handleDelete(l.id)} canDelete={budget.status === "draft"}
...
function ViewRow({ line, i, bands, showAlloc, onEdit, onDelete, canDelete, onWorking, hasWorking, isOwnWorking }: {
