...
// Read-only graph VIEW tab of the control plane. Editing lives in the table tabs; this is the
// picture/lens — see connections + spot broken ones (red). Lifted from the old graph page.
...
export default function GraphTab() {
  ...
  const gNodes: GraphNode[] = (graph?.nodes ?? []).map((n) => ({ id: n.id, label: n.label, sublabel: n.sublabel, kind: n.kind, status: KIND_STYLE[n.kind].label, broken: n.broken }));
  const gEdges: GraphEdge[] = (graph?.edges ?? []).map((e) => ({ id: e.id, from: e.from, to: e.to, broken: e.broken }));

  return (
    <div>
      <div className="flex items-center gap-3 mb-3">
        <p className="text-xs text-stone-400 flex-1">A read-only view of the live config graph. Edit in the other tabs.</p>
        <label className="flex items-center gap-1.5 text-xs text-stone-500 cursor-pointer select-none">
          <input type="checkbox" checked={connectedOnly} onChange={(e) => setConnectedOnly(e.target.checked)} className="accent-stone-700" /> Connected only
        </label>
      ...
      {graph && graph.brokenCount > 0 && (
        <div className="mb-3 flex items-center gap-2 px-4 py-2.5 bg-red-50 border border-red-200 rounded-xl text-sm text-red-700">
          ...
          <span><b>{graph.brokenCount}</b> broken connection{graph.brokenCount === 1 ? "" : "s"} — a binding points at a checklist that no longer exists. Fix it in the Indicators or Templates tab.</span>
        </div>
      )}

      <div className="flex flex-wrap items-center gap-3 mb-3 text-[11px] text-stone-500">
        {Object.entries(KIND_STYLE).map(([k, s]) => (
          <span key={k} className="inline-flex items-center gap-1.5"><span className="w-3 h-3 rounded" style={{ background: s.bg, border: `1.5px solid ${s.border}` }} />{s.label}</span>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-3">
        <GraphCanvas
          ...
          minHeight={520}
          hint="Click a node to inspect · read-only"
        />
        <div className="border border-stone-200 rounded-xl bg-white p-4 h-fit lg:sticky lg:top-4">
          {selected ? (
            <div className="space-y-2">
              ...
              {selected.broken && <div className="text-xs text-red-600 flex items-center gap-1"><AlertTriangle className="w-3.5 h-3.5" /> Referenced but missing — fix in the table tabs.</div>}
              {selected.href && <a href={selected.href} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs text-sky-600 hover:text-sky-800 pt-1">Open editor <ExternalLink className="w-3 h-3" /></a>}
            </div>
          ) : <p className="text-xs text-stone-400">Click a node to inspect it.</p>}
        </div>
      </div>
    </div>
  );
