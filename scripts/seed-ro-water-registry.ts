/**
 * Seed the parametric RO_Water CostRegistry items + their component working
 * (CostRegistryComponent). Sourced from lib/ro-water/rates.ts so budget +
 * model stay aligned.
 *
 ...
 *
 * Usage:
 *   npx tsx scripts/seed-ro-water-registry.ts             # dry run
 *   npx tsx scripts/seed-ro-water-registry.ts --apply     # write
 *   npx tsx scripts/seed-ro-water-registry.ts --apply --chennai  # Chennai too
 */
import { PrismaClient } from "../app/generated/prisma/client";
import { PrismaPg } from "@prisma/adapter-pg";
import { rollup } from "../lib/budget/costComponents";
import { RO_WATER_RATES } from "../lib/ro-water/rates";
import * as dotenv from "dotenv";
dotenv.config({ path: ".env.local" });

const APPLY = process.argv.includes("--apply");
const INCLUDE_CHENNAI = process.argv.includes("--chennai");
const DOMAIN = "RO_Water";

async function seedForCity(prisma: PrismaClient, city: string) {
  console.log(`\n=== RO_Water registry seed (${city}) — ${APPLY ? "APPLY" : "DRY RUN"} ===`);
  ...
  for (const rate of RO_WATER_RATES) {
    if (APPLY) {
      await prisma.costRegistry.upsert({
        ...
        // in-place admin edits survive re-runs.
        update: {
          domain: DOMAIN,
          unit: rate.costUnit,
          notes: rate.notes ?? null,
    ...
    console.log(`  ${rate.key.padEnd(30)} ${status}  (${rate.components.length} components, Σ=${sum})`);
